      ...
      <div className={`mx-auto ${reading ? 'max-w-5xl' : ''} p-4 sm:p-6`}>
        ...
        {!loading && paste && <>
          ...
          <div className="mb-4 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4 lg:grid-cols-7">{[
            ...
            ['Created', new Date(`${paste.created_at.replace(' ', 'T')}Z`).toLocaleDateString()], ['Updated', new Date(`${paste.updated_at.replace(' ', 'T')}Z`).toLocaleDateString()], ['Expires', expires === null ? 'Never' : expires < 60_000 ? '< 1 min' : `${Math.ceil(expires / 60_000)} min`],
