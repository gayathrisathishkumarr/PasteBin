...
export default function App() {
  ...
  async function uploadAsDraft(file?: File) { if (!file) return; if (file.size > 100_000) return setNotice('Choose a text file under 100 KB.'); const content = await file.text(); localStorage.setItem(storageKeys.draft, JSON.stringify({ title: file.name.replace(/\.[^.]+$/, ''), description: '', content, language: 'Plain Text', visibility: prefs.visibility, tags: [], expiresAt: null })); navigate('new'); }
  ...
  return <div className="min-h-screen bg-dark-950 font-sans text-white">
    ...
    {secretMeta && <div className="modal-backdrop"><section role="alertdialog" aria-modal="true" className="glass-card max-w-md p-6"><div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-amber-500/10 text-amber-300"><Trash2 /></div><h2 className="text-xl font-bold">Burn-after-reading secret</h2><p className="mt-2 text-sm leading-6 text-gray-400">“{secretMeta.title}” will be permanently deleted as soon as it opens. It can only be retrieved once.</p><div className="mt-5 flex justify-end gap-2"><button onClick={closePaste} className="btn-secondary">Cancel</button><button onClick={() => { const id = secretMeta.id; setSecretMeta(null); void openPaste(id, false, true); }} className="btn-primary !from-amber-600 !to-orange-600">Open and burn</button></div></section></div>}
    {deleteConfirm && <div className="modal-backdrop"><section role="alertdialog" aria-modal="true" className="glass-card max-w-md p-6"><h2 className="text-xl font-bold">{deleteConfirm.title}</h2><p className="mt-2 text-sm leading-6 text-gray-400">{deleteConfirm.message}</p><div className="mt-5 flex justify-end gap-2"><button onClick={() => setDeleteConfirm(null)} className="btn-secondary">Cancel</button><button onClick={() => { const action = deleteConfirm.action; setDeleteConfirm(null); void action().catch((e) => setNotice(errorMessage(e))); }} className="btn-primary !from-red-600 !to-rose-600">Delete</button></div></section></div>}
    {importPreview && <div className="modal-backdrop"><section role="dialog" aria-modal="true" aria-labelledby="import-title" className="glass-card w-full max-w-lg p-6"><h2 id="import-title" className="text-xl font-bold">Preview JSON import</h2><p className="mt-2 text-sm text-gray-400">The API will validate all {importPreview.length} pastes before saving.</p><ul className="mt-4 max-h-56 space-y-2 overflow-y-auto">{importPreview.map((item, index) => <li key={`${item.title}-${index}`} className="rounded-lg border border-white/[0.05] p-3"><p className="truncate text-sm font-medium">{item.title || 'Untitled (invalid)'}</p><p className="mt-1 text-xs text-gray-600">{item.language || 'Unknown'} · {item.content?.length || 0} characters</p></li>)}</ul><div className="mt-5 flex justify-end gap-2"><button onClick={() => setImportPreview(null)} className="btn-secondary">Cancel</button><button onClick={() => void confirmImport()} className="btn-primary">Validate and import</button></div></section></div>}
