...
export default function App() {
  ...
  const heroLanguages = Array.from(new Set((lineage?.nodes ?? []).map((node) => node.language))).slice(0, 4);
    ...
    <div className="min-h-screen lg:ml-[250px]"><TopNav search={search} onSearch={(value) => { setSearch(value); setPage(1); }} onCreate={() => navigate('new')} onPalette={() => setPalette(true)} showSearch={['pastes','explore','favorites'].includes(view)} />
      <main id="main-content" className={`p-4 sm:p-6 ${prefs.density === 'compact' ? 'density-compact' : ''}`}>
        {view === 'dashboard' && <div className="mx-auto max-w-7xl space-y-6">
          <section className="hero-panel relative overflow-hidden rounded-2xl border border-violet-500/15 p-5 sm:p-7"><div aria-hidden="true" className="absolute -right-20 -top-20 h-60 w-60 rounded-full border border-violet-400/10 shadow-[0_0_100px_rgba(124,58,237,.14)]" /><div className="relative grid items-center gap-5 lg:grid-cols-[1fr_380px]"><div><p className="flex items-center gap-2 text-sm font-semibold text-violet-400"><Sparkles className="h-4 w-4" />Your developer knowledge base</p><h1 className="mt-2 max-w-2xl text-3xl font-bold tracking-tight sm:text-5xl">Forge snippets.<br/><span className="text-gradient">Trace understanding.</span></h1><p className="mt-3 max-w-xl text-sm leading-6 text-gray-400">A focused workspace where code can be found, remixed, versioned—and traced back to the ideas that shaped it.</p><div className="mt-4 flex flex-wrap gap-3"><button onClick={() => navigate('new')} className="btn-primary">Create a paste</button><button onClick={() => document.getElementById('lineage-title')?.scrollIntoView({ behavior: prefs.reducedMotion ? 'auto' : 'smooth' })} className="btn-secondary flex items-center gap-2">Explore code lineage <ArrowRight className="h-4 w-4" /></button></div></div><div className="relative hidden min-h-40 lg:block"><div className="absolute left-1/2 top-1/2 h-32 w-32 -translate-x-1/2 -translate-y-1/2 rounded-full border border-violet-400/20"><div className="absolute inset-4 rounded-full border border-dashed border-cyan-400/20"><div className="absolute inset-6 flex items-center justify-center rounded-full border border-white/[0.08] bg-violet-500/10"><Code2 className="h-7 w-7 text-violet-300" /></div></div></div>{heroLanguages.length ? heroLanguages.map((label, index) => { const points = ['left-3 top-5','right-8 top-1','bottom-2 left-12','bottom-6 right-3']; return <span key={label} className={`absolute ${points[index]} max-w-[120px] rounded-lg border border-white/10 bg-dark-900/80 px-2.5 py-1.5 text-center font-mono text-[10px] text-gray-300 shadow-xl`}>{label}</span>; }) : <span className="absolute right-8 top-3 rounded-lg border border-white/10 bg-dark-900/80 px-3 py-1.5 font-mono text-sm text-gray-500">{'< />'}</span>}<span className="absolute bottom-0 left-1/2 flex -translate-x-1/2 items-center gap-2 whitespace-nowrap text-[10px] text-gray-500"><Network className="h-3 w-3 text-cyan-400" />{lineage ? <>{countLabel(lineage.meta.totalNodes, 'snippet')} · {countLabel(lineage.meta.totalEdges, 'discovered link')}</> : 'Tracing real relationships…'}</span></div></div></section>
